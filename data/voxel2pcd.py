...
def voxel2pcd(npy_sec, dir_pcd_dep, dir_pcd_sec):
    ...
    if np.array(np.where(voxel_sec > 0)).size * np.array(np.where(voxel_dep > 0)).size > 0:
        ...
        write_point_cloud(dir_pcd_sec + npy_sec[name_start:name_end] + '0.pcd', pcd)
        ...
        write_point_cloud(dir_pcd_dep + npy_dep[name_start:name_end] + '0.pcd', pcd)
