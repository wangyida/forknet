    ...
    def build_model(self):
                        ...
                        1)) + tf.reduce_mean(
                            tf.reduce_sum(
                                tf.nn.sigmoid_cross_entropy_with_logits(
                                    ...
                                    labels=tf.zeros_like(h_gen_dec_vox)), 1))
                        ...
                        1)) + tf.reduce_mean(
                            tf.reduce_sum(
                                tf.nn.sigmoid_cross_entropy_with_logits(
                                    ...
                                    labels=tf.zeros_like(h_gen_dec_tsdf)), 1))
        ...
        gen_loss += tf.reduce_mean(
            tf.reduce_sum(
                tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=h_gen_dec_tsdf,
                    labels=tf.ones_like(h_gen_dec_tsdf)), 1))
        ...
        # Cost
        cost_enc = 0.1 * code_encode_loss + self.lamda_recons * (
            recons_vae_loss + recons_cc_loss)
        cost_gen = self.lamda_recons * (
            recons_vae_loss + recons_cc_loss) + 10 * gen_loss
        cost_discrim = 10 * discrim_loss
        cost_code = 0.1 * code_discrim_loss
