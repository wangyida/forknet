...
class FCR_aGAN():
    def __init__(
        ...
        self.gen_W1 = tf.Variable(
                tf.random_normal([
                    ...
                    self.dim_W1*self.start_vox_size[0]*self.start_vox_size[1]*self.start_vox_size[2]], 
                    stddev=0.02), 
                name='gen_W1')
        ...
        self.gen_bn_g2 = tf.Variable( 
                tf.random_normal(
                    ...
                    mean=1.0, stddev=0.02), 
                name='gen_bn_g2')
        self.gen_bn_b2 = tf.Variable( 
                tf.zeros([self.dim_W2]), 
                name='gen_bn_b2')
        ...
        self.gen_bn_g3 = tf.Variable( 
                tf.random_normal([
                    ...
                    stddev=0.02), 
                name='gen_bn_g3')
        self.gen_bn_b3 = tf.Variable( 
                tf.zeros([self.dim_W3]), 
                name='gen_bn_b3')
        ...
        self.gen_bn_g4 = tf.Variable( 
                tf.random_normal([self.dim_W4], 
                    mean=1.0, stddev=0.02), 
                name='gen_bn_g4')
        self.gen_bn_b4 = tf.Variable( 
                tf.zeros([self.dim_W4]), 
                name='gen_bn_b4')
        ...
        self.gen_bn_g5 = tf.Variable( 
                tf.random_normal([self.dim_W5], 
                    mean=1.0, stddev=0.02), 
                name='gen_bn_g5')
        self.gen_bn_b5 = tf.Variable( 
                tf.zeros([self.dim_W5]), 
                name='gen_bn_b5')
        ...
        self.encode_bn_g4 = tf.Variable( 
                tf.random_normal([self.dim_W1], 
                    mean=1.0, stddev=0.02), 
                name='encode_bn_g4')
        self.encode_bn_b4 = tf.Variable( 
                tf.zeros([self.dim_W1]), 
                name='encode_bn_b4')

        self.encode_W5 = tf.Variable(
                tf.random_normal([1,1,1,self.dim_W1,self.dim_z], 
                    stddev=0.02), 
                name='encode_W5')
        ...
        # depth--start
        self.encode_dep_W1 = tf.Variable(
                tf.random_normal([
                    self.kernel5[0],
                    self.kernel5[1],
                    1,
                    self.dim_W4], 
                    stddev=0.02), 
                name='depthproject_W1')
        self.encode_dep_bn_g1 = tf.Variable( 
                tf.random_normal([self.dim_W4], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g1')
        self.encode_dep_bn_b1 = tf.Variable( 
                tf.zeros([self.dim_W4]), 
                name='depthproject_bn_b1')

        self.encode_dep_W2 = tf.Variable(
                tf.random_normal([
                    self.kernel4[0],
                    self.kernel4[1],
                    self.dim_W4,
                    self.dim_W3], 
                    stddev=0.02), 
                name='depthproject_W2')
        self.encode_dep_bn_g2 = tf.Variable( 
                tf.random_normal([self.dim_W3], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g2')
        self.encode_dep_bn_b2 = tf.Variable( 
                tf.zeros([self.dim_W3]), 
                name='depthproject_bn_b2')

        self.encode_dep_W3 = tf.Variable(
                tf.random_normal([
                    self.kernel3[0],
                    self.kernel3[1],
                    self.dim_W3,
                    self.dim_W2], 
                    stddev=0.02), 
                name='depthproject_W3')
        self.encode_dep_bn_g3 = tf.Variable( 
                tf.random_normal([self.dim_W2], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g3')
        self.encode_dep_bn_b3 = tf.Variable( 
                tf.zeros([self.dim_W2]), 
                name='depthproject_bn_b3')

        self.encode_dep_W4 = tf.Variable(
                tf.random_normal([
                    self.kernel2[0],
                    self.kernel2[1],
                    self.dim_W2,
                    self.dim_W1], 
                    stddev=0.02), 
                name='depthproject_W4')
        self.encode_dep_bn_g4 = tf.Variable( 
                tf.random_normal([self.dim_W1], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g4')
        self.encode_dep_bn_b4 = tf.Variable( 
                tf.zeros([self.dim_W1]), 
                name='depthproject_bn_b4')

        self.encode_dep_W5 = tf.Variable(
                tf.random_normal([
                    self.kernel2[0],
                    self.kernel2[1],
                    self.dim_W1,
                    self.dim_W1], 
                    stddev=0.02), 
                name='depthproject_W5')
        self.encode_dep_bn_g5 = tf.Variable( 
                tf.random_normal([self.dim_W1], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g5')
        self.encode_dep_bn_b5 = tf.Variable( 
                tf.zeros([self.dim_W1]), 
                name='depthproject_bn_b5')

        self.encode_dep_W6 = tf.Variable(
                tf.random_normal([
                    self.kernel2[0],
                    self.kernel2[1],
                    self.dim_W1,
                    256], 
                    stddev=0.02), 
                name='depthproject_W6')
        self.encode_dep_bn_g6 = tf.Variable( 
                tf.random_normal([256], 
                    mean=1.0, stddev=0.02), 
                name='depthproject_bn_g6')
        self.encode_dep_bn_b6 = tf.Variable( 
                tf.zeros([256]), 
                name='depthproject_bn_b6')
        self.encode_dep_W7 = tf.Variable(
                tf.random_normal([
                    5*4*256, 
                    self.start_vox_size[0]*self.start_vox_size[1]*self.start_vox_size[2]*self.dim_W1], 
                    stddev=0.02), 
                name='depthproject_W7')

        self.encode_dep_W8 = tf.Variable(
                tf.random_normal([
                    1,1,1,self.dim_W1,self.dim_z], 
                    stddev=0.02), 
                name='depthproject_W8')
        self.encode_dep_W8_sigma = tf.Variable(
                tf.random_normal([
                    1,1,1,self.dim_W1,self.dim_z], 
                    stddev=0.02), 
                name='depthproject_W8_sigma')
        # depth--end
        ...
        self.discrim_dep_bn_b4 = tf.Variable( 
                tf.zeros([1]), 
                name='discrim_dep_bn_b4')

        self.discrim_dep_W5 = tf.Variable(
                tf.random_normal([
                    self.start_vox_size[0]*self.start_vox_size[1]*self.start_vox_size[2]*self.dim_W1,1], 
                    stddev=0.02), 
                name='discrim_dep_W5')
        # depth--end
        ...
        self.cod_bn_g1 = tf.Variable( 
                tf.random_normal([dim_code], 
                    mean=1.0, stddev=0.02), 
                name='cod_bn_g1')
        self.cod_bn_b1 = tf.Variable( 
                tf.zeros([dim_code]), 
                name='cod_bn_b1')

        self.cod_W2 = tf.Variable(
                tf.random_normal([dim_code, dim_code], 
                    stddev=0.02), 
                name='cod_W2')
        self.cod_bn_g2 = tf.Variable( 
                tf.random_normal([dim_code], 
                    mean=1.0, stddev=0.02), 
                name='cod_bn_g2')
        self.cod_bn_b2 = tf.Variable( 
                tf.zeros([dim_code]), 
                name='cod_bn_b2')

        self.cod_W3 = tf.Variable(
                tf.random_normal([dim_code, 1], 
                    stddev=0.02), 
                name='cod_W3')        
    ...
    def build_model(self):
        ...
        # depth--start
        dep_real = tf.placeholder(tf.float32, [self.batch_size, self.dep_shape[0], self.dep_shape[1], self.dep_shape[2]])
        # depth--end
        ...
        code_encode_loss = tf.reduce_mean(tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_code_encode, labels=tf.ones_like(h_code_encode)), [1]))
        ...
        recons_dep_loss = -tf.reduce_sum(self.lamda_gamma *vox_real * tf.log(1e-6 + vox_gen_decode_dep) + (1- self.lamda_gamma) * (1-vox_real) * tf.log(1e-6 + 1-vox_gen_decode_dep), [1,2,3]) 
        recons_dep_loss = tf.reduce_mean(tf.reduce_sum(recons_dep_loss * weight, 1))
        #Refiner
        ...
        #Standard_GAN_Loss
        discrim_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_real, labels=tf.ones_like(h_real)))\
         + tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_gen, labels=tf.zeros_like(h_gen))) + tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_gen_dec, labels=tf.zeros_like(h_gen_dec)))
        ...
        gen_loss_refine = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_gen_ref, labels=tf.ones_like(h_gen_ref))) + tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=h_gen_dec_ref, labels=tf.ones_like(h_gen_dec_ref)))
    ...
    def encoder_dep(self, dep):
        ...
        h1 = lrelu( tf.nn.conv2d( dep, self.encode_dep_W1, strides=self.stride_dep, padding='SAME' ))
        h2 = lrelu( batchnormalize( tf.nn.conv2d( h1, self.encode_dep_W2, strides=self.stride_dep, padding='SAME'), g=self.encode_dep_bn_g2, b=self.encode_dep_bn_b2, batch_size=self.batch_size) )
        h3 = lrelu( batchnormalize( tf.nn.conv2d( h2, self.encode_dep_W3, strides=self.stride_dep, padding='SAME'), g=self.encode_dep_bn_g3, b=self.encode_dep_bn_b3, batch_size=self.batch_size) )
